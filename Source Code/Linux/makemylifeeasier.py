...
def makemylifeeasier():
        ...
        config.read('/home/'+getpass.getuser()+'/Scripts/pairs.conf')
	...
	#Also create analysis folders
		...
		MATLAB_ANALYSIS_SCRIPT = config.get (y,'matlab_analysis_script')
			...
			print PATH+'dcm2nii '+os.path.abspath('')+'/'+DATA+'/'+data_file[0]
					...
					#find file names of volumes to be discarded
                        ...
                        if (MATLAB_PREPROCESS_SCRIPT == ''):
                                ...
                                try:
                                        ...
                                        filetry=open('/home/'+getpass.getuser()+'/Scripts/'+MATLAB_PREPROCESS_SCRIPT+'.m')
                                except IOError:
                        ...
                        if (MATLAB_ANALYSIS_SCRIPT == ''):
                                ...
                                try:
                                        #check if script file is present
                                        filetry=open('/home/'+getpass.getuser()+'/Scripts/'+MATLAB_ANALYSIS_SCRIPT+'.m')
                                except IOError:
                ...
                x=x+1
        if (JOB == ''):
                print 'Job folder not specified. Skipping creation'
        else:
                try:
                        os.makedirs(JOB)
                except OSError:
...
def makefile():
        ...
        while (pair_number > x):
                ...
                if not os.path.isfile('/home/'+getpass.getuser()+'/Scripts/pairs.conf'):
                        #file doesn't exist. Add first pair data to it
                        file = open('/home/'+getpass.getuser()+'/Scripts/pairs.conf', 'w+')
                        file.write('['+y+']\n')
                        ...
                        file = open('/home/'+getpass.getuser()+'/Scripts/pairs.conf', 'a')
                        file.write('['+y+']\n')
                        file.write('data_folder='+data_folder+'\n')
                        file.write('analysis_folder='+analysis_folder+'\n')
                        file.write('kind_of='+kind_of+'\n')
        ...
        path = raw_input('Enter the folder where dcm2nii is located. Your MRICRON folder :')
        file = open('/home/'+getpass.getuser()+'/Scripts/pairs.conf', 'a')
        file.write('[path]\n')
...
try:			
        filetry=open('/home/'+getpass.getuser()+'/Scripts/pairs.conf')
